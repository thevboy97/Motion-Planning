...
def runtest(mapfile, robotstart, targetstart):
    ...
    # initiliaze environment class for storing different parameters
    # change algo value to implement different A* variants
    env = environment(envmap, robotpos, targetpos, algo=0)
    ...
    for i in range(20000):
        ...
        # print('iter', i)
        t0 = tic()
        newrobotpos = env.robotplanner(envmap, robotpos, targetpos)

        rx = [robotpos[0], newrobotpos[0]]
        ry = [robotpos[1], newrobotpos[1]]
        plt.plot(rx, ry, 'blue')
        ...
        tx = [targetpos[0], newtargetpos[0]]
        ty = [targetpos[1], newtargetpos[1]]
        plt.plot(tx, ty, 'red', linestyle='dashed')
